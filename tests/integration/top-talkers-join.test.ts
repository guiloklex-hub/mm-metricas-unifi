...
describe('listTopTalkers JOIN clients', () => {
  ...
  beforeEach(() => {
    db = createTestDb();
    // FK obriga controller/site existirem antes de inserir client. Cria
    // fixtures mínimas para isolar o teste do pipeline de cadastro real.
    db.$client
      .prepare(
  ...
  it('traz hostname/name/alias quando cliente está no catálogo', () => {
    ...
    // Insere métricas direto para os 2 clientes.
    const insert = db.$client.prepare(
      `INSERT INTO metrics_5m (ts, controller_id, site_id, device_id, radio, client_mac,
         client_count, tx_bytes, tx_packets, d_tx_bytes, d_tx_packets)
       VALUES (?, ?, ?, '', '', ?, 1, NULL, NULL, ?, ?)`,
    );
    insert.run(ts, CTRL, SITE, 'aa:bb:cc:11:22:33', 1_000_000, 1000);
    insert.run(ts, CTRL, SITE, 'aa:bb:cc:11:22:99', 500_000, 500); // sem catálogo

    const rows = listTopTalkers(db, { from: ts - 60, to: ts + 60 });
